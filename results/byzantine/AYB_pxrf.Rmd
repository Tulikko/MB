...
biplot(pca_2, choices = 1:2, cex = c(1, 1.2), col = c("grey80", "deeppink2"), main = "PCA Ashdod-Yam Byzantine elements (NO COPPER)")

autoplot(pca_1, data=pxrf_final, colour='Type', shape = FALSE, label = TRUE,  main = "PCA Ashdod-Yam Byzantine grouped by sample type")
autoplot(pca_2, data=pxrf_noCu, colour='Type', shape = FALSE, label = TRUE,  main = "PCA Ashdod-Yam Byzantine grouped by sample type (NO COPPER)")
...
dend2 <- 
    ...
    plot(dend2, main="HCA with sample types (NO COPPER)")
    rect.dendrogram(dend2, 5, border = "Black", lty = 5)
    legend("topright", legend = levels(type), fill = cols_t)
